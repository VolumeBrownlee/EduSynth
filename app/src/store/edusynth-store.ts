...
export function buildAchievements(profile: Profile, progress: StudyProgressItem[]): Achievement[] {
  ...
  return [
    /* Icon strings map to Lucide components in AchievementsView. */
    { id: 'a1',  title: 'First Steps',     description: 'Complete your first module',           icon: 'Rocket',     category: 'learning', unlocked: masteredCount >= 1,                              progress: Math.min(masteredCount, 1),                                                                       maxProgress: 1,    xpReward: 100 },
    { id: 'a2',  title: 'Curious Mind',    description: 'Ask 50 Socratic questions',            icon: 'HelpCircle', category: 'learning', unlocked: totalQueries >= 50,                              progress: Math.min(totalQueries, 50),                                                                       maxProgress: 50,   xpReward: 200 },
    { id: 'a3',  title: 'Deep Thinker',    description: 'Ask 200 Socratic questions',           icon: 'Brain',      category: 'learning', unlocked: totalQueries >= 200,                             progress: Math.min(totalQueries, 200),                                                                      maxProgress: 200,  xpReward: 500 },
    { id: 'a4',  title: 'Trailblazer',     description: 'Reach a 7-day streak',                 icon: 'Flame',      category: 'streak',   unlocked: profile.streak_count >= 7,                       progress: Math.min(profile.streak_count, 7),                                                                maxProgress: 7,    xpReward: 150 },
    { id: 'a5',  title: 'Unstoppable',     description: 'Reach a 30-day streak',                icon: 'Zap',        category: 'streak',   unlocked: profile.streak_count >= 30,                      progress: Math.min(profile.streak_count, 30),                                                               maxProgress: 30,   xpReward: 1000 },
    { id: 'a6',  title: 'Scholar',         description: 'Master 3 modules',                     icon: 'BookOpen',   category: 'mastery',  unlocked: masteredCount >= 3,                              progress: Math.min(masteredCount, 3),                                                                       maxProgress: 3,    xpReward: 300 },
    { id: 'a7',  title: 'Sage',            description: 'Master 10 modules',                    icon: 'GraduationCap', category: 'mastery', unlocked: masteredCount >= 10,                            progress: Math.min(masteredCount, 10),                                                                     maxProgress: 10,   xpReward: 1000 },
    { id: 'a8',  title: 'Night Owl',       description: 'Study for 10 hours total',             icon: 'Moon',       category: 'special',  unlocked: totalTime >= 600,                                progress: Math.min(totalTime, 600),                                                                         maxProgress: 600,  xpReward: 250 },
    { id: 'a9',  title: 'Challenge Ready', description: 'Reach 70% Ready-Score on any module',  icon: 'Swords',     category: 'mastery',  unlocked: progress.some((p) => p.ready_score >= 70),       progress: progress.some((p) => p.ready_score >= 70) ? 1 : 0,                                                maxProgress: 1,    xpReward: 200 },
    { id: 'a10', title: 'XP Hunter',       description: 'Earn 5,000 XP',                        icon: 'Gem',        category: 'special',  unlocked: profile.xp_points >= 5000,                       progress: Math.min(profile.xp_points, 5000),                                                                maxProgress: 5000, xpReward: 500 },
    { id: 'a11', title: 'Perfectionist',   description: 'Score 90%+ on any module quiz',        icon: 'Trophy',     category: 'mastery',  unlocked: progress.some((p) => p.quiz_avg >= 90),          progress: progress.some((p) => p.quiz_avg >= 90) ? 1 : 0,                                                   maxProgress: 1,    xpReward: 400 },
    { id: 'a12', title: 'Renaissance',     description: 'Study across 3 different courses',     icon: 'Sparkles',   category: 'social',   unlocked: new Set(progress.map((p) => p.classroom_id)).size >= 3, progress: Math.min(new Set(progress.map((p) => p.classroom_id)).size, 3),                            maxProgress: 3,    xpReward: 350 },
  ];
}
